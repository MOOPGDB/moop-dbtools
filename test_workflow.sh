...
perl parsers/parse_GFF3_to_MOOP_TSV.pl \
  "$TEST_DATA/genomic.gff3" \
  "$TEST_DATA/organisms.tsv" \
...
perl loaders/load_genes_sqlite.pl \
  --schema \
  --db "$DB_FILE" \
  --genus "$GENUS" \
  --species "$SPECIES"
...
perl parsers/parse_DIAMOND_to_MOOP_TSV.pl \
  "$TEST_DATA/UNIPROT_sprot.tophit.tsv" \
  uniprot_sprot \
  > "$OUTPUT_DIR/diamond_annotations.tsv"
echo "✓ Created: $OUTPUT_DIR/diamond_annotations.tsv"

echo "Loading DIAMOND annotations into database..."
perl loaders/load_annotations_sqlite.pl \
  --db "$DB_FILE" \
  --file "$OUTPUT_DIR/diamond_annotations.tsv" \
...
perl parsers/parse_InterProScan_to_MOOP_TSV.pl \
  "$TEST_DATA/iprscan_results.tsv" \
  > "$OUTPUT_DIR/interproscan_annotations.tsv"
echo "✓ Created: $OUTPUT_DIR/interproscan_annotations.tsv"

echo "Loading InterProScan annotations into database..."
perl loaders/load_annotations_sqlite.pl \
  --db "$DB_FILE" \
  --file "$OUTPUT_DIR/interproscan_annotations.tsv" \
  --source "InterProScan" \
  --version "5.52.0"
